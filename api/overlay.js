...
export default handle(async req=>{
 ...
 // Widget delivery must survive an expired Twitch token. Report its health
 // separately so a Twitch outage cannot hide Doneru/Streamlabs notifications.
 let twitchStatus={ok:true};
 try{await credentials(account);}catch(e){twitchStatus={ok:false,error:e.message};}
 ...
 return json({events:batch,now:batch.length?Math.max(...batch.map(e=>e.receivedAt)):Date.now(),stream:{...stream,streaks},hype:await get('hype:'+account),widgets:u.searchParams.get('widgets')==='1'?{doneru:integrations?.doneru||'',streamlabs:integrations?.mode==='api'?'':integrations?.streamlabs||''}:undefined,twitchStatus,streamlabsStatus:await get('sl-status:'+account)});
});
