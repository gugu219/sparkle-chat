(() => {
  ...
  async function editor(){
    const form=document.querySelector('#widget-form'),frame=document.querySelector('#widget-preview'),out=document.querySelector('#obs-url'),toast=document.querySelector('#toast'),key='prism-widget-id';let widget=qs.get('widget')||localStorage.getItem(key)||'',auth=await session(widget);if(qs.get('widget')){localStorage.setItem(key,widget);history.replaceState({},'',location.pathname);}if(auth&&!form.elements.channel.value)form.elements.channel.value=auth.login;
    const values=()=>({...defaults,...Object.fromEntries(new FormData(form)),blur:form.elements.blur.checked?'1':'0'});const url=(preview=false)=>{const p=new URLSearchParams(values());if(widget)p.set('widget',widget);if(preview)p.set('preview','1');return `${new URL('view.html',location.href).href}?${p}`;};let timer;const update=()=>{const s=values();apply(s);out.value=url();clearTimeout(timer);timer=setTimeout(()=>frame.src=url(true),500);document.querySelector('#opacity-value').textContent=`${s.opacity}%`;};
    const authUi=()=>{const ok=!!auth;document.querySelector('#auth-title').textContent=ok?`Connected: ${auth.login}`:'Twitch not connected';document.querySelector('#auth-copy').textContent=ok?`Connected as ${auth.login}. Sign-in is optional for read-only overlays.`:'公開チャンネルのチャットは認証なしで表示できます。接続すると自分のログインで参加します。';document.querySelector('#twitch-login').hidden=ok;document.querySelector('#twitch-disconnect').hidden=!ok;document.querySelector('#connection-status').textContent=ok?`Connected: #${auth.login}`:'Test mode';};
