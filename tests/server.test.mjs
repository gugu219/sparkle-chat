...
test('upload and config writes reject anonymous callers',async()=>{assert.equal((await image(request('/api/image','POST',{data:'data:image/png;base64,AAAA'}))).status,500);assert.equal((await configApi(request('/api/config','POST',{config:{}}))).status,500);});
test('config rejects credentials and unsaved local frames',async()=>{await set('owner:owner','account');for(const cfg of [{alert:{widget:'old-token'}},{frame:{customImage:'data:image/png;base64,AA'}}])assert.equal((await configApi(request('/api/config','POST',{config:cfg},true))).status,400);});
